...
`Prompt Quest` is a browser-based, RPG-style prompt delivery map for middle school students. Students explore three zones, open a themed prompt drawer, copy a prompt, and bring it into Gemini to build, understand, or debug their web app ideas. The experience is designed for touch-first Chromebook use, with an added keyboard-play mode where a pixel panther can move around the map with `WASD` and activate zones with `Space`.
...
The app runs from these four files:
